...
#define PRO_JAG 1 /* generov n¡ vìstup… pro Jagu r */
...
static Err FZapisCekani( PreklInfo *PI, long Cas )
{
	if( Cas>0 )
	{
		while( Cas>0x7fff )
		{
			...
			if( PI->GenJag )
			{
				fprintf(PI->GenJag,"\tdc.w pSync<<8,$7fff\n");
			}
			#endif
		}
		if( !ZapisW(&PI->PisBuf,(MSync<<8)|0) ) return ERAM;
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.w pSync<<8,$%lx\n",Cas);
		}
		#endif
	}
	return EOK;
...
static Err FOutKan( PreklInfo *PI, AlokKan *PA )
{
	...
	if( PI->GenJag )
	{
		fprintf(PI->GenJag,"\tdc.b pNotaZac,$%x,%d,$ff\n",PA->Kanal,PA->Ton);
	}
	#endif
	return EOK;
}
...
static Err VypKan( PreklInfo *PI, AlokKan *PA )
{
	if( PA->Ton>=0 ) /* hraje */
	{
		...
		if( !PA->CekamLegato )
		{
			...
			if( PI->GenJag )
			{
				fprintf(PI->GenJag,"\tdc.b pNotaZac,$%x,%d,0\n",PA->Kanal,PA->Ton);
			}
			#endif
		}
...
static Err ZapKan( PreklInfo *PI, AlokKan *PA, int LogTon, int Vol, Flag Legato, CasT Cas )
{
	...
	if( PA->CekamLegato )
	{
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.b pNotaZac,$%x,",PA->Kanal+0x40);
		}
		#endif
	}
	else
	{
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.b pNotaZac,$%x,",PA->Kanal);
		}
		#endif
	}
	...
	{ /* Rytmika nazabira cas */
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.b pNotaZac,%d,%d,0\n",PA->Kanal,LogTon);
		}
		#endif
		PA->Ton=-1;
		if( PA->back ) *PA->back=NULL;
...
static Err SetNastr( PreklInfo *PI, AlokKan *PA, KanPrekl *K, int V, int Ton )
{
	...
	if( PA->LNastroj!=V )
	{
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.w (pNastr<<8)|%d,ins_%s\n",PA->Kanal,NN);
		}
		#endif
		PA->LNastroj=V;
		PA->FNastroj=F;
...
static Err SetEfekt( PreklInfo *PI, AlokKan *PA, int EA, int EB )
{
	if( PA->EffA!=EA || PA->EffB!=EB )
	{
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.b pEfekt,%d,%d,%d\n",PA->Kanal,EA,EB);
		}
		#endif
	}
	return EOK;
...
static Err SetStereo( PreklInfo *PI, AlokKan *PA, int VMin, int VMax )
{
	if( PA->StereoMin!=VMin || PA->StereoMax!=VMax )
	{
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.b pStereo,%d,%d,%d\n",PA->Kanal,VMin,VMax);
		}
		#endif
	}
	return EOK;
...
static Err Preloz( Sekvence *S, CasT CasOd, Kontext *K, PreklInfo *PI )
{
	...
	if( ret>=EOK )
	{
		...
		if( PI->GenJag )
		{
			fprintf(PI->GenJag,"\tdc.b pKonec,0,0,0\n");
		}
		#endif
	}
		...
		if( p>=OK )
		{
			...
			PInfo.GenJag=NULL;
				...
				if( !Hraj )
				{
					...
					strcpy(NajdiPExt(N),"");
					strcpy(NajdiPExt(NajdiNazev(C)),".S");
					VyrobCesty(CP,CN,C);
					if( !*CocPath ) strcpy(CocPath,CP);
					else strcpy(CP,CocPath);
					if( FSel(CocPath,CN,C) ) PInfo.GenJag=fopen(C,"w");
					if( PInfo.GenJag )
					{
						fprintf(PInfo.GenJag,"; Componium (Jaguar)\n");
						fprintf(PInfo.GenJag,"; ½ 1996, ä SUMA\n\n");
						fprintf(PInfo.GenJag,"include \"music.inc\"\n\n");
						fprintf(PInfo.GenJag,"data\n\n",N);
						fprintf(PInfo.GenJag,"%s::\n",N);
					}
					FAktHNastr();
				}
			#endif
