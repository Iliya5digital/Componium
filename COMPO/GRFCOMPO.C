...
typedef struct
{
	...
	enum tramsmer {TJedno,TDolu,TNahoru,TRadDolu,TRadNahoru} TramSmer; /* kterìm smØrem mus¡ dostat nohu */
} GrfStatus;
...
{
	byte *off=(byte *)&VRA[(ypos-yosnova+10)*((long)BPL>>1)];
	char h[16];
	sprintf(h,"%d",K->KteryTakt+1+PridejTaktKont(K));
	return TiskS8(off,h,xpos)+4;
}
...
static int Zjistixpos( OknoInfo *OI, int FKan, int offs, CasT CasPos )
{
	...
	for( Kan=0; Kan<OI->NStop; Kan++ )
	{
		...
		for( of=0; of<NOff; of++ ) if( Kan!=FKan || of!=offs )
		{
			...
			ef( I->xcas> CasPos ) xp=I->zpos+CDif;
			ef( I->xcas==CasPos ) xp=I->xpos;
...
static void UzavriTram( GrfStatus *KS )
...
static void KresliTramH( int x1, int y1, int x2, int y2, int Cet )
{
	...
	ef( dy<0 ) ddy=(dy-0x10000L)/(dx+1),y+=0xffff;
	else ddy=0;
	VA+=(x1>>3);
	for( ; x1<=x2; x1++ )
	{
...
static void KresliTramD( int x1, int y1, int x2, int y2, int Cet )
{
	long dy=(long)(y2-y1)<<16;
	int dx=x2-x1; /* x2>x1 */
	long y=(long)y1<<16;
...
static void ZavedTram( GrfStatus *KS, int x, int y, int Cet, enum tramsmer Smer )
{
	if( !KS ) /* hned nakresli */
	{
		switch( Smer )
	...
	else
	{
		...
		if( KS->TramX>=0 ) /* zkus to na nØkoho napojit */
		{
			if( KS->NTram==Cet && abs(y-KS->TramY)*2<=(x-KS->TramX) )
			{
				...
				ef( KS->TramSmer==TDolu && Smer!=TNahoru ) Nahoru=False;
				ef( Smer==TDolu && KS->TramSmer!=TNahoru ) Nahoru=False;
				ef( Smer==TNahoru && KS->TramSmer!=TDolu ) Nahoru=True;
				ef( Smer==TRadNahoru || KS->TramSmer==TRadNahoru ) Nahoru=True;
				else Nahoru=False;
				if( !Nahoru )
				{
...
static int FKresliPau( WindRect *W, JednItem *B, int xpos, int ypos, int off )
{
	Sprite *I;
	int y;
	int DelVys,DelAtr;
...
static int FKresliTonD( GrfStatus *KS, WindRect *W, JednItem *B, int xpos, int ypos, Flag Prapor, Flag Musi )
{
	Sprite *I;
	int DelVys,DelAtr;
	int PropPom;
	enum CPovel Druh=B[0];
...
static int FKresliTonH( GrfStatus *KS, WindRect *W, JednItem *B, int xpos, int ypos, Flag Prapor, Flag Musi )
{
	Sprite *I;
	int DelVys,DelAtr;
	enum CPovel Druh=B[0];
...
static int FKresliTon( GrfStatus *KS, WindRect *W, JednItem *B, int xpos, int ypos )
{
...
static int CmpVNota( const VNota *v1, const VNota *v2 )
{
...
static int KresliVec( GrfStatus *KS, Vector *V, int xpos, int ypos, OknoInfo *OI )
{
	int W=0;
	KS->PridanTram=False;
...
static int KresliTon( GrfStatus *KS, WindRect *W, SekvSoubor *s, int xpos, int ypos, int Kan, OknoInfo *OI )
{
	JednItem *B=BufS(s);
	Vector *V=&TonVec[Kan];
	if( V->n<MaxVec )
	...
	if( !(B[1]&Soubezna) )
	{
		...
		int Prop=KresliVec(KS,V,xpos,ypos,OI);
...
static void KresliKomin( byte *off, int x, int w, int h )
{
	while( --h>=0 )
	{
		HCara(off,x,x+w-1);
...
static void TiskEff( byte *off, int xpos, int EA )
{ /* nakresl¡ kom¡n */
	int H;
	int HK=7;
	H=(EA*HK+500)/1000;
	KresliKomin(off+BPL*(long)(HK-H),xpos,4,H+1);
}
...
static void KresliBlok( int Kan, int xz, int xk )
{
	int ypos=kanypos(Kan);
	if( xk>xmax ) xk=xmax;
	HBlok(&VRA[(ypos-ystep*6)*((long)BPL/2)],xz,xk,ystep*12);
}
...
static Flag JeSynchro( enum CPovel Druh )
{
	if( Druh<=PPau ) return True;
	ef( Druh==PVyvolej ) return True;
	else return False;
}

#define YNasOff ( -4 )

static void KresliJedn( SekvSoubor *s, int xpos, int Kan, OknoInfo *OI, int offs )
{
	GrfStatus *KN=&KIStat[Kan];
	JednItem *SB=BufS(s);
	enum CPovel Druh=StatusS(s)!=EOK ? ERR : SB[0];
	int ypos=kanypos(Kan);
	int Prop;
	int xpos0=xpos;
	WindRect *W=NULL;
	...
	ef( Druh<=PPau ) /* ton */
	{
		Prop=KresliTon(KN,W,s,xpos,ypos,Kan,OI);
	...
	else /* Povel */
	{
		switch( Druh )
		{
			...
			case PNTakt:
			{
				byte *off=(byte *)&VRA[(ypos-yosnova+16)*((long)BPL>>1)];
				char h[4],d[4];
				int xpo;
				sprintf(h,"%d",SB[1]);
				sprintf(d,"%d",SB[2]);
				...
				TiskS(&off[0*BPL],h,xpo);
				TiskS(&off[12*BPL],d,xpo);
				TiskS(&off[36*BPL],h,xpo);
				TiskS(&off[48*BPL],d,xpo);
				Prop=xpo-xpos+10;
				W->x=xpos;
				W->y=yhorr+6;
				W->h=hcele-6;
			...
			case PTempo:
			{
				byte *off=(byte *)&VRA[(ypos-yosnova+YNasOff)*((long)BPL>>1)];
				char h[4];
				int xpo,xp;
				sprintf(h,"=%d",SB[1]);
				UzavriTram(KN);
			...
			case POktava:
			{
				byte *off=(byte *)&VRA[(ypos-yosnova+YNasOff+8)*((long)BPL>>1)];
				char h[16];
				int xpo;
				if( SB[2] ) sprintf(h,"%+d",SB[1]);
				else sprintf(h,"*%+d",SB[1]);
				xpo=KrZacOktav(xpos,ypos);
				Prop=xpo-xpos+TiskS8(off,h,xpo)+4;
				W->x=xpos;
				W->y=ypos-yosnova-2;
				W->h=16;
				W->w=Prop-4;
			...
			case PHlasit:
			{
				byte *off=(byte *)&VRA[(ypos+ystep*4-2)*((long)BPL>>1)];
				const char *N=HlasitText(SB[1]);
				Prop=TiskS8T(off,N,xpos+1)+4;
				W->x=xpos;
				W->y=ypos+ystep*4-2;
				W->h=8;
				W->w=Prop-4;
			...
			case PEfekt:
			{
				...
				byte *off=(byte *)&VRA[ye*((long)BPL>>1)];
				Prop=TiskS8(off," ",xpos)+4;
				TiskEff(off,xpos,SB[1]);
				TiskEff(off,xpos+4,SB[2]);
				W->x=xpos;
				W->y=ye;
				W->h=8;
				W->w=Prop-4;
			...
			case PZacSekv:
			{
				...
				byte *off=(byte *)&VRA[(ypos-yosnova+40)*((long)BPL>>1)];
				char N[40];
				int xpo;
				UzavriTram(KN);
			...
			{
				int xpo;
				byte *off=(byte *)&VRA[(ypos-yosnova+44)*((long)BPL>>1)];
				char h[4];
				UzavriTram(KN);
	...
	}
	{
		...
		(
			...
			Druh!=PUmPauza && JeVBloku(KOI,Kan+KOI->IStopa,&LPos[Kan],KS->i[offs].zcas)
		)
		{ /* je blok */
...
static void Osnova( int y )
{
	...
	for( i=0; i<Klice2Spr.s_h; i++ )
	{
		...
		*PS++=*IM;
		...
		((word *)IM)+=Klice2Spr.s_w;
	}
}

static void ZacniKresl( OknoInfo *OI, SekvSoubor *ss )
{
	int Kan;
	VRA=PracScr;
	memset(PracScr,0,(long)BPL*ScrH);
	OI->KR.x=OI->KR.y=-1;
	OI->KR.h=OI->KR.w=0;
	...
	for( Kan=0; Kan<OI->NStop; Kan++ )
	{
		...
		if( JeVBloku(OI,Kan+OI->IStopa,&LPos[Kan],CP) )
		{
			if( CP==0 ) KS->ZBlok=0;
...
static void XorRect( const WindRect *W )
{
	...
	int xb=W->x;
	int xe=xb+W->w-1;
	int ye=max(W->y,0)+W->h;
	if( ye>=ScrH ) ye=ScrH;
	bmask=bm[xb&7];
	emask=em[xe&7];
	xb>>=3;
	xe>>=3;
	Adr+=xb+max(W->y,0)*((long)BPL);
	xe-=xb;
	if( xe<=0 )
	{
		bmask&=emask;
...
static void FKresliNoty( int LHan )
{
	OknoInfo *OI=OInfa[LHan];
	SekvSoubor ss[NMaxStop];
	int xpos;
	int Kan;
	xmax=Okna[LHan]->Uvnitr.w;
	/* kresleni osnovy */
	KOI=OI;
	ZacniKresl(OI,ss);
	/* kresleni not */
			...
			if( KS->stat==OK )
			{
				...
				if( CP>=c )
				{
					...
					else /* CP==c */
					{
						...
						ef( d==0 && UzSek[MKan]<UzSek[Kan] ) continue;
						ef( (PP=PovelPred(Pov,PA))>=0 )
						{
							if( PP>0 ) CD=d,Pov=PA,MKan=Kan;
							ef( UzRep[MKan]>UzRep[Kan] ) CD=d,Pov=PA,MKan=Kan; /* PP==0 */
							ef( UzRep[MKan]==UzRep[Kan] )
							{
...
Flag HledejKursor( int LHan, int x, int y, int HMode, int *Ins )
{
	...
	for( PP=Prot[Kan],NP=PPC[Kan]-1; NP>=0; NP--,PP++ )
	{
		...
		if( HMode!=HKursPred )
		{
			...
			ef( PP->WR.x<=x-4 && NP>0 ) PI=PP,PIZa=True; /* jeste zname dalsiho */
		}
		else /* HKursPred */
		{
